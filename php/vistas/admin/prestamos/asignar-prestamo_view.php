...
$usuarios = $obj_usuarios -> consultAllUsersButCurrent($_SESSION["userProfile"]["id"]);

// if(isset($_SESSION["prestamo"])){
//     unset($_SESSION["prestamo"]);
// }

if(isset($_SESSION["nuestroPrestamoAsignado"])) {
...
if(isset($_SESSION["asigning-prestamo"]) && $estado["estado"] == "En proceso") { 
    echo "
    <script>
    setTimeout( function() {
...
<div class="page">
    <div class="page__navigation">
        ...
        <a href="<?php echo RUTA;?>historial-prestamos" class="page__card">
            ...
            <div class="page__card-block">
                ...
                <span>Examina los prestamos realizados</span>
            </div>
            <svg class="page__card-icon page__card-icon--right">
    ...
    <div id="iniciar_prestamo" class="form-container select-allowed <?php if($estado["estado"] == "Devuelto" || $estado["estado"] == "Cancelado"){ echo "show "; } if(isset($_SESSION["asigning-prestamo"])){ echo "hidden"; }?>" style="padding: 0; padding-top: 4rem; margin-bottom: 2rem">
        <form class='inside-form'>
            <h2 class='form__heading'>Asigna un prestamo</h2>
            ...
            <?php
                ...
                if(mysqli_num_rows($aulas) >= 1) {
                    if(mysqli_num_rows($usuarios) >= 1){
                        ...
                        <label for='prestamoAula' class='page__heading' style='display: block; font-size: calc(var(--font-app) + .1rem); text-align: left; margin-left: .3rem; margin-bottom: .5rem; margin-top: 1rem'>Usuario a asignar el prestamo</label>
                        <select id='prestamoUser' required placeholder='Hello'>";
                            foreach($usuarios as $fila){
                                $print.= "<option value='$fila[id_usuario]'>$fila[nom_usuario] $fila[ape_usuario]</option>";
                            }
    
                            $print.="
                        </select>
                        <button type='button' id='asign-prestamo' class='form__btn no-margin' style='margin-top: 1.5rem !important'>Asignar nuevo prestamo</button>
                        ";
                    } else {
                        $print.= "
                        <div class='warning-depto' style='margin-bottom: 1rem'>
                            <p>No existe ningun usuario al cual asignar el prestamo.</p>
                        </div>
                        ";    
                    }
                } else {
                    $print.= "
                    <div class='warning-depto' style='margin-bottom: 1rem'>
                        <p>No existe ninguna aula para asignar al prestamos.</p>
                    </div>
                    ";
                }
    ...
    <div id="agregar__equipos" class="table-container select-allowed <?php if(!isset($_SESSION["asigning-prestamo"]) || ($estado["estado"] == "Devuelto" || $estado["estado"] == "Cancelado")){ echo "hidden"; }?>">
        <div class="page__division" sty></div>
        <h2 class='page__heading' style="font-size: calc(var(--font-app) + 1rem);">Agrega materiales al prestamo asignado</h2>
    ...
    <div id="equipos_agregados" class="table-container select-allowed <?php if(!isset($_SESSION["asigning-prestamo"]) || ($estado["estado"] == "Devuelto" || $estado["estado"] == "Cancelado")){ echo "hidden"; }?>" style="margin-top: 2rem">
        <div class="page__division" sty></div>
        <h2 class='page__heading' style="font-size: calc(var(--font-app) + 1rem);">Materiales agregados al prestamo asignado</h2>
        <div class="page__division" style="margin-bottom: 4rem;"></div> 
...
<script src="js/admin/prestamos/asignar-prestamos.js"></script>
